...
namespace System.Yaml.Serialization
{
    ...
    {
        private readonly Dictionary<Type, Func<object>> activators =
    ...
    {
        public static YamlTagResolver TagResolver = new YamlTagResolver();
        ...
        private object NodeToObjectInternal(YamlNode node, Type expected, Dictionary<YamlNode, object> appeared)
        {
            ...
            // Type resolution
            var type = expected == typeof (object) ? null : expected;
            var fromTag = TagResolver.TypeFromTag(node.Tag);
            if (fromTag == null)
        ...
        private object ScalarToObject(YamlScalar node, Type type)
        {
            if (type == null)
        ...
        private object SequenceToObject(YamlSequence seq, Type type, Dictionary<YamlNode, object> appeared)
        {
            if (type == null)
                type = typeof (object[]);
        ...
        private void SetArrayElements(Array array, YamlSequence seq, int i, int[] indices, Type elementType,
            Dictionary<YamlNode, object> appeared)
        {
        ...
        private object MappingToObject(YamlMapping map, Type type, object obj, Dictionary<YamlNode, object> appeared)
        {
            // Naked !!map is constructed as Dictionary<object, object>.
            if (((map.ShorthandTag() == "!!map" && type == null) || type == typeof (Dictionary<object, object>)) &&
                obj == null)
            {
                var dict = new Dictionary<object, object>();
                appeared.Add(map, dict);
                foreach (var entry in map)
                    dict.Add(NodeToObjectInternal(entry.Key, null, appeared),
                        NodeToObjectInternal(entry.Value, null, appeared));
                return dict;
            }
                ...
                switch (name)
                {
                    ...
                    case "IDictionary.Entries":
                        ...
                        foreach (var child in (YamlMapping) entry.Value)
                            dict.Add(NodeToObjectInternal(child.Key, access.KeyType, appeared),
                                NodeToObjectInternal(child.Value, access.ValueType, appeared));
                        break;
                    default:
                        ...
                        switch (access[name].SerializeMethod)
                        {
                            ...
                            case YamlSerializeMethod.Content:
                                MappingToObject((YamlMapping) entry.Value, access[name].Type, access[obj, name],
                                    appeared);
                                break;
