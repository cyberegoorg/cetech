...
namespace System.Yaml
{
    ...
    {
        private static readonly DoubleQuote DoubleQuotedString = new DoubleQuote();
        private static readonly Regex ForbiddenChars = new Regex(@"[\x00-\x08\x0B\x0C\x0E-\x1F]");
        private static readonly Regex OneLine = new Regex(@"^([^\n\r]|\n)*(\r?\n|\r)?$");
        private static readonly Regex UntilBreak = new Regex(@"[^\r\n]*(\r?\n|\r)");

        private static readonly YamlTagResolver AutoTagResolver = new YamlTagResolver();
        private static readonly YamlParser PlainChecker = new YamlParser();
        // has a tag handle and the body contains only ns-tag-char.
        private static readonly Regex CanBeShorthand =
            new Regex(@"^!([-0-9a-zA-Z]*!)?[-0-9a-zA-Z%#;/?:@&=+$_.^*'\(\)]*$");

        private int column;
        private int raw;
        private YamlConfig config;
        private TextWriter yaml;
        ...
        private static void MarkMultiTimeAppearingChildNodesToBeAnchored(YamlNode node)
        {
        ...
        internal static string NextAnchor(string anchor) // this is "protected" for test use 
        {
            ...
            {
                return anchor.Substring(0, anchor.Length - 1) + (char) (anchor[anchor.Length - 1] + 1);
            }
        ...
        private void ScalarToYaml(YamlScalar node, string pres, Context c)
        {
            ...
            if (IsValidPlainText(s, c) &&
                !(node.ShorthandTag() == "!!str" && auto_tag != null && !node.Properties.ContainsKey("plainText")))
            {
        ...
        private bool WriteImplicitKeyIfPossible(YamlNode node, string pres, Context c)
        {
            if (!(node is YamlScalar))
                return false;
            var raw_saved = raw;
            var col_saved = column;
            var yaml_saved = yaml;
            var result = "";
        ...
        private class DoubleQuote : Parser<DoubleQuote.State>
        {
            private readonly Dictionary<char, string> CharEscaping = new Dictionary<char, string>();

            private readonly Func<char, bool> nbDoubleSafeCharset = Charset(c =>
                (0x100 <= c && c != '\u2028' && c != '\u2029') ||
                c == 0x09 ||
            ...
            {
                Parse(() => DoubleQuoteString(pres, c), s);
                return "\"" + stringValue + "\"";
            }

            private bool DoubleQuoteString(string pres, Context c)
            {
                return Repeat(() => cDoubleQuoteChar(pres, c));
            }

            private bool cDoubleQuoteChar(string pres, Context c)
            {
                return
                    !EndOfString() && (
                        nbDoubleSafeChar() ||
