...
namespace System.Yaml
{
    ...
    public class YamlConfig
    {
        ...
        ///     </para>
        ///     <para>
        ///         Then, whenever it is possible, the <see cref="YamlNode" />s are presented
        ...
        ///     </para>
        ///     <para>
        ///         To suppress normalization of line breaks by YAML parser, set <see cref="NormalizeLineBreaks" />
        ...
        /// <param name="decode">
        ///     Method that decode value from <see cref="Match" />
        ///     data after matching by <paramref name="regex" />.
        /// </param>
        ...
        {
            TagResolver.AddRule(tag, regex, decode, encode);
        }
    ...
    ///     <para>
    ///         See <see cref="YamlScalar" />, <see cref="YamlSequence" /> and <see cref="YamlMapping" />
    ...
    ///     <para>
    ///         See <a href="http://yaml.org/">http://yaml.org/</a> for the official definition of
    ...
    ///     <para>
    ///         In reality, a <see cref="YamlNode" /> is either <see cref="YamlScalar" />, <see cref="YamlSequence" /> or
    ///         <see cref="YamlMapping" />.
    ///     </para>
    ///     <para>
    ///         All <see cref="YamlNode" /> has <see cref="YamlNode.Tag" /> property that denotes
    ///         the actual data type represented in the YAML node.
    ///     </para>
    ///     <para>
    ///         Default Tag value for <see cref="YamlScalar" />, <see cref="YamlSequence" /> or <see cref="YamlMapping" /> are
    ///         <c>"tag:yaml.org,2002:str"</c>, <c>"tag:yaml.org,2002:seq"</c>, <c>"tag:yaml.org,2002:map"</c>.
    ///     </para>
    ///     <para>
    ///         Global tags that starts with <c>"tag:yaml.org,2002:"</c> ( =
    ///         <see cref="YamlNode.DefaultTagPrefix">
    ///             YamlNode.DefaultTagPrefix
    ///         </see>
    ///         ) are defined in the YAML tag repository at
    ///         <a href="http://yaml.org/type/">http://yaml.org/type/</a>. In this library, such a tags can be also
    ///         represented in a short form that starts with <c>"!!"</c>, like <c>"!!str"</c>, <c>"!!seq"</c> and
    ///         <c>"!!map"</c>.
    ...
    ///         <c>"!!float"</c> and <c>"!!timestamp"</c> tags, by default.
    ///     </para>
    ///     <para>
    ///         <see cref="YamlNode" />s can be read from a YAML stream with <see cref="YamlNode.FromYaml(string)" />,
    ...
    ///     </para>
    ///     <para>
    ///         <see cref="YamlNode" />s can be written to a YAML stream with <see cref="YamlNode.ToYaml()" />,
    ///         <see cref="YamlNode.ToYaml(Stream)" />, <see cref="YamlNode.ToYaml(TextWriter)" /> and
    ///         <see cref="YamlNode.ToYamlFile(string)" />.
    ///     </para>
    ///     <para>
    ...
    ///     </para>
    ///     <para>
    ///         A <see cref="YamlScalar" /> has <see cref="YamlScalar.Value" /> property, which holds the string expression
    ///         of the node value.
    ///     </para>
    ///     <para>
    ///         A <see cref="YamlSequence" /> implements <see cref="IList{T}">IList&lt;YamlNode&gt;</see>
    ///         interface to access the child nodes.
    ///     </para>
    ///     <para>
    ///         <see cref="YamlMapping" /> implements
    ...
    ///     <para>
    ///         Implicit cast operators from <see cref="string" />, <see cref="bool" />, <see cref="int" />,
    ...
    ///         as arguments in addition to <see cref="YamlNode" /> types.
    ///     </para>
    ///     <code>
    ...
    ///     <para>
    ///         Equality of <see cref="YamlNode" />s are evaluated on the content base. Different <see cref="YamlNode" />
    ///         objects that have the same content are evaluated to be equal. Use <see cref="Equals(object)" /> method for
    ///         equality evaluation.
    ///     </para>
    ///     <para>
    ///         In detail, two <see cref="YamlNode" />s are logically equal to each other when the <see cref="YamlNode" />
...
//        static YamlTagValidator TagValidator = new YamlTagValidator();
        ...
        {
            private readonly Dictionary<YamlNode, int> nodes_a =
                new Dictionary<YamlNode, int>(TypeUtils.EqualityComparerByRef<YamlNode>.Default);

            private readonly Dictionary<YamlNode, int> nodes_b =
                new Dictionary<YamlNode, int>(TypeUtils.EqualityComparerByRef<YamlNode>.Default);

            private readonly Stack<YamlNode> stack_a = new Stack<YamlNode>();
            private readonly Stack<YamlNode> stack_b = new Stack<YamlNode>();
            ...
            {
                int ai, bi;
                var ar = nodes_a.TryGetValue(a, out ai);
                var br = nodes_b.TryGetValue(b, out bi);
                if (ar && br && ai == bi)
                {
                    identity = true;
                    return true;
        ...
        private bool HashInvalid = true;
        private bool ToBeRehash;
        ...
        {
            return new YamlScalar("!!int", DefaultConfig.TypeConverter.ConvertToString(value));
        }
        ...
        {
            return new YamlScalar("!!float", DefaultConfig.TypeConverter.ConvertToString(value));
        }
        ...
        {
            return new YamlScalar("!!bool", DefaultConfig.TypeConverter.ConvertToString(value));
        }
    ...
    public class YamlScalar : YamlNode
    {
        ...
        ///         refer to <see cref="NativeObjectAvailable" />.
        ///     </para>
        /// </summary>
        /// <exception cref="InvalidOperationException">This property is not available. See <see cref="NativeObjectAvailable" />.</exception>
        /// <remarks>
        ///     <para>
        ///         This property is available when <see cref="YamlNode.DefaultConfig" />.<see cref="YamlConfig.TagResolver" />
        ///         contains
        ///         an entry for the nodes tag and defines how to decode the <see cref="Value" /> property into native objects.
        ///     </para>
        ///     <para>
        ///         When this property is available, equality of the scalar node is evaluated by comparing the
        ///         <see cref="NativeObject" />
        ...
        {
            bool skip;
            if (!EqualsSub(b, repository, out skip))
                return false;
            if (skip)
                return true;
            var aa = this;
        ...
        {
            Tag = ExpandTag("!!int");
            Value = DefaultConfig.TypeConverter.ConvertToString(value);
        }
    ...
    {
        private readonly RehashableDictionary<YamlNode, YamlNode> mapping =
        ...
        internal override bool Equals(YamlNode b, ObjectRepository repository)
        {
            ...
            foreach (var item in this)
            {
                ...
                var theone = new KeyValuePair<YamlNode, YamlNode>();
        ...
        private void CheckDuplicatedKeys()
        {
            foreach (var entry in this)
                CheckDuplicatedKeys(entry.Key);
        }

        private void CheckDuplicatedKeys(YamlNode key)
        {
        ...
        private void ChildRemoved(object sender, RehashableDictionary<YamlNode, YamlNode>.DictionaryEventArgs e)
        {
        ...
        private void ChildAdded(object sender, RehashableDictionary<YamlNode, YamlNode>.DictionaryEventArgs e)
        {
            e.Key.Changed += KeyChanged;
            e.Value.Changed += ChildChanged;
            OnChanged();
        ...
        private void ProcessMergeKey()
        {
            // find merge key
            var merge_key = Keys.FirstOrDefault(key => key.Tag == ExpandTag("!!merge"));
            if (merge_key == null)
                return;
        ...
        private void Merge(YamlMapping map)
        {
        ...
        /// <returns>
        ///     true if the <see cref="YamlMapping" /> contains an element with the key that is equal to the specified value;
        ///     otherwise, false.
        /// </returns>
        ...
        ///     found in the original <see cref="YamlMapping" />.
        /// </returns>
        ...
        /// </param>
        /// <returns>
        ///     true if the object that implements <see cref="YamlMapping" /> contains an element with the specified key;
        ///     otherwise, false.
        /// </returns>
        ...
        void ICollection<KeyValuePair<YamlNode, YamlNode>>.CopyTo(KeyValuePair<YamlNode, YamlNode>[] array,
            int arrayIndex)
        {
    ...
    public class YamlSequence : YamlComplexNode, IList<YamlNode>, IDisposable
    {
        ...
        private void OnItemAdded(YamlNode item)
        {
            item.Changed += ItemChanged;
        }

        private void OnItemRemoved(YamlNode item)
        {
            item.Changed -= ItemChanged;
        }
        ...
        private List<YamlNode> sequence = new List<YamlNode>();
        ...
        ///     <see cref="YamlSequence" />.
        /// </exception>
        /// <remarks>
        ///     <para>
        ///         If <paramref name="index" /> equals the number of items in the <see cref="YamlSequence" />,
        ///         then <paramref name="item" /> is appended to the sequence.
        ///     </para>
        ...
        /// <remarks>
        ///     <para>
        ///         This property provides the ability to access a specific node in the sequence by using the following syntax:
        ///         mySequence[index].
        ///     </para>
        /// </remarks>
        ...
        /// <summary>
        ///     Copies the child nodes of the <see cref="YamlSequence" /> to an <see cref="Array" />, starting at a particular
        ...
        ///     <para>-or-</para>
        ///     <para>
        ///         The number of elements in the source <see cref="YamlSequence" /> is greater than the available space from
        ...
        /// <returns>
        ///     true if <paramref name="node" /> was successfully removed from the <see cref="YamlSequence" />; otherwise, false.
        ///     This method also returns false if <paramref name="node" /> is not found in the original <see cref="YamlSequence" />
        ///     .
        /// </returns>
