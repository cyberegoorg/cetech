...
namespace scene_blob {
    ...
    inline const uint32_t blob_size(const blob_t* blob) {
        return (
            ...
            ( sizeof(uint32_t) * blob->geom_count ) +
            ( sizeof(ct_render_vertex_decl_t) * blob->geom_count ) +
            ( sizeof(uint32_t) * blob->geom_count ) +
            ( sizeof(uint32_t) * blob->geom_count ) +
    ...
    inline ct_render_vertex_decl_t* vb_decl(const blob_t* blob) {
        return ((ct_render_vertex_decl_t*) (vb_offset(blob) + (blob->geom_count)));
    }
