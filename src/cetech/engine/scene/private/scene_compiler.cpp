...
static void _parse_vertex_decl(ct_render_vertex_decl_t *decl,
    ...
    *vertex_size += int(size) * v_size;
    ...
    ct_renderer_a0.vertex_decl_add(decl, type, (uint8_t) int(size), attrib_type, 0, 0);
}
...
static int _compile_assimp(const char *filename,
    ...
    for (uint32_t i = 0; i < scene->mNumMeshes; ++i) {
        ...
        if (mesh->mVertices != NULL) {
            ct_renderer_a0.vertex_decl_add(&vertex_decl,CT_RENDER_ATTRIB_POSITION, 3,
                            CT_RENDER_ATTRIB_TYPE_FLOAT, 0, 0);
            v_size += 3 * sizeof(float);
        }

        if (mesh->mNormals != NULL) {
            ct_renderer_a0.vertex_decl_add(&vertex_decl,CT_RENDER_ATTRIB_NORMAL, 3,
                                           CT_RENDER_ATTRIB_TYPE_FLOAT, true, 0);
            v_size += 3 * sizeof(float);
        }

        if (mesh->mTextureCoords[0] != NULL) {
            ct_renderer_a0.vertex_decl_add(&vertex_decl,CT_RENDER_ATTRIB_TEXCOORD0, 2,
                                           CT_RENDER_ATTRIB_TYPE_FLOAT, 0, 0);
