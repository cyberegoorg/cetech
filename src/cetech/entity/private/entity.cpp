...
static void write_to_build(ct_entity_compile_output *output,
    ...
    cel_array_push_n(*build, &res, sizeof(struct entity_resource),
                     _G.allocator);

    //write guids
    cel_array_push_n(*build, &output->guid[0], sizeof(uint64_t) * res.ent_count,
                     _G.allocator);
    ...
    cel_array_push_n(*build,
                     ...
                     sizeof(uint64_t) * cel_array_size(output->component_type),
                     _G.allocator);
    ...
    for (uint32_t j = 0; j < res.comp_type_count; ++j) {
        ...
        cel_array_push_n(*build, (uint8_t *) &cdata, sizeof(cdata),
                         _G.allocator);
        cel_array_push_n(*build, (uint8_t *) ent_arr,
                         sizeof(uint32_t) * cdata.ent_count, _G.allocator);
        cel_array_push_n(*build, blob, sizeof(uint8_t) * cel_array_size(blob),
                         _G.allocator);
