...
void set_module(void *inst,
                struct ct_rg_module_t0 *module) {
    struct ct_rg_t0 *rg = inst;
    struct render_graph_o *rg_inst = rg->inst;
...
static struct ct_rg_t0 *create_render_graph() {
    struct ct_rg_t0 *obj = CE_ALLOC(_G.alloc, ct_rg_t0, sizeof(ct_rg_t0));

    struct render_graph_o *inst = CE_ALLOC(_G.alloc,
                                           render_graph_o,
...
static void destroy_render_graph(ct_rg_t0 *render_graph) {
    struct ct_rg_t0 *rg = render_graph;
    struct render_graph_o *rg_inst = rg->inst;
