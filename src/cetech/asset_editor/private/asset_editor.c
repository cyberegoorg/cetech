...
static struct ct_asset_editor_i0 *get_asset_editor(uint64_t cdb_type) {
    ce_api_entry_t0 it = ce_api_a0->first(CT_ASSET_EDITOR_I0);
    while (it.api) {
...
//    }
    ...
    if (ei->top_level) {
        ct_asset_editor_i0 *i = get_asset_editor(ei->type);
...
static void draw_editor(uint64_t content,
    ...
    ct_asset_editor_i0 *i = get_asset_editor(editor->type);
...
void CE_MODULE_LOAD(editor)(struct ce_api_a0 *api,
    ...
    ct_dock_a0->create_dock(ASSET_EDITOR, true);
}
