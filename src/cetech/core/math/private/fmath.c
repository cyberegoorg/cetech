...
uint32_t ct_float_to_bits(float _a) {
    union {
        float f;
        uint32_t ui;
    } u = {_a};
...
float ct_bits_to_float(uint32_t _a) {
    union {
        uint32_t ui;
        float f;
    } u = {_a};
...
uint64_t ct_double_to_bits(double _a) {
    union {
        double f;
        uint64_t ui;
    } u = {_a};
...
double ct_bits_to_double(uint64_t _a) {
    union {
        uint64_t ui;
        double f;
    } u = {_a};
...
float ct_fabsolute(float _a) {
    return fabsf(_a);
}

float ct_fsin(float _a) {
    return sinf(_a);
}

float ct_fasin(float _a) {
    return asinf(_a);
}

float ct_fcos(float _a) {
    return cosf(_a);
}

float ct_ftan(float _a) {
    return tanf(_a);
}

float ct_facos(float _a) {
    return acosf(_a);
}

float ct_fatan2(float _y,
                float _x) {
    return atan2f(_y, _x);
}

float ct_fpow(float _a,
              float _b) {
    return powf(_a, _b);
}

float ct_flog(float _a) {
    return logf(_a);
}

float ct_fsqrt(float _a) {
    return sqrtf(_a);
}

float ct_ffloor(float _f) {
    return floorf(_f);
}

float ct_fceil(float _f) {
    return ceilf(_f);
}

float ct_fmod(float _a,
              float _b) {
    return fmodf(_a, _b);
}

bool ct_is_nan(float _f) {
    const uint32_t tmp = ct_float_to_bits(_f) & INT32_MAX;
    return tmp > UINT32_C(0x7f800000);
}

bool ct_is_nand(double _f) {
    const uint64_t tmp = ct_double_to_bits(_f) & INT64_MAX;
    return tmp > UINT64_C(0x7ff0000000000000);
}

bool ct_is_finite(float _f) {
    const uint32_t tmp = ct_float_to_bits(_f) & INT32_MAX;
    return tmp < UINT32_C(0x7f800000);
}

bool ct_is_finited(double _f) {
    const uint64_t tmp = ct_double_to_bits(_f) & INT64_MAX;
    return tmp < UINT64_C(0x7ff0000000000000);
}

bool ct_is_infinite(float _f) {
    const uint32_t tmp = ct_float_to_bits(_f) & INT32_MAX;
    return tmp == UINT32_C(0x7f800000);
}

bool ct_is_infinited(double _f) {
    const uint64_t tmp = ct_double_to_bits(_f) & INT64_MAX;
    return tmp == UINT64_C(0x7ff0000000000000);
}

float ct_fround(float _f) {
    return ct_ffloor(_f + 0.5f);
}

float ct_fmin(float _a,
              float _b) {
    return _a < _b ? _a : _b;
}

float ct_fmax(float _a,
              float _b) {
    return _a > _b ? _a : _b;
}

float ct_fmin3(float _a,
               float _b,
               float _c) {
    return ct_fmin(_a, ct_fmin(_b, _c));
}

float ct_fmax3(float _a,
               float _b,
               float _c) {
    return ct_fmax(_a, ct_fmax(_b, _c));
}

float ct_fclamp(float _a,
                float _min,
                float _max) {
    return ct_fmin(ct_fmax(_a, _min), _max);
}

float ct_fsaturate(float _a) {
    return ct_fclamp(_a, 0.0f, 1.0f);
}

float ct_flerp(float _a,
               float _b,
               float _t) {
    return _a + (_b - _a) * _t;
}

float ct_fsign(float _a) {
    return _a < 0.0f ? -1.0f : 1.0f;
}

float ct_fsq(float _a) {
    return _a * _a;
}

float ct_fexp2(float _a) {
    return ct_fpow(2.0f, _a);
}

float ct_flog2(float _a) {
    return ct_flog(_a) * CT_INV_LOG_NAT_2;
}

float ct_frsqrt(float _a) {
    return 1.0f / ct_fsqrt(_a);
}

float ct_ffract(float _a) {
    return _a - ct_ffloor(_a);
}

bool ct_fequal(float _a,
               float _b,
               float _epsilon) {
...
bool ct_fequal_n(const float *_a,
                 const float *_b,
                 uint32_t _num,
                 float _epsilon) {
...
void ct_vec3_move(float *_result,
                  const float *_a) {
    _result[0] = _a[0];
    _result[1] = _a[1];
    _result[2] = _a[2];
...
void ct_vec3_abs(float *_result,
                 const float *_a) {
...
void ct_vec3_neg(float *_result,
                 const float *_a) {
    _result[0] = -_a[0];
    _result[1] = -_a[1];
    _result[2] = -_a[2];
...
void ct_vec3_add(float *_result,
                 const float *_a,
                 const float *_b) {
    _result[0] = _a[0] + _b[0];
    _result[1] = _a[1] + _b[1];
...
void ct_vec3_add_s(float *_result,
                   const float *_a,
                   float _b) {
    _result[0] = _a[0] + _b;
    _result[1] = _a[1] + _b;
...
void ct_vec3_sub(float *_result,
                 const float *_a,
                 const float *_b) {
    _result[0] = _a[0] - _b[0];
    _result[1] = _a[1] - _b[1];
...
void ct_vec3_sub_s(float *_result,
                   const float *_a,
                   float _b) {
    _result[0] = _a[0] - _b;
    _result[1] = _a[1] - _b;
...
void ct_vec3_mul(float *_result,
                 const float *_a,
                 const float *_b) {
    _result[0] = _a[0] * _b[0];
    _result[1] = _a[1] * _b[1];
...
void ct_vec3_mul_s(float *_result,
                   const float *_a,
                   float _b) {
    _result[0] = _a[0] * _b;
    _result[1] = _a[1] * _b;
...
float ct_vec3_dot(const float *_a,
                  const float *_b) {
    return _a[0] * _b[0] + _a[1] * _b[1] + _a[2] * _b[2];
}

void ct_vec3_cross(float *_result,
                   const float *_a,
                   const float *_b) {
...
float ct_vec3_length(const float *_a) {
    return ct_fsqrt(ct_vec3_dot(_a, _a));
}

void ct_vec3_lerp_s(float *_result,
                    const float *_a,
                    const float *_b,
                    float _t) {
...
void ct_vec3_lerp(float *_result,
                  const float *_a,
                  const float *_b,
                  const float *_c) {
...
float ct_vec3_norm(float *_result,
                   const float *_a) {
...
void ct_vec3_min(float *_result,
                 const float *_a,
                 const float *_b) {
...
void ct_vec3_max(float *_result,
                 const float *_a,
                 const float *_b) {
...
void ct_vec3_rcp(float *_result,
                 const float *_a) {
    _result[0] = 1.0f / _a[0];
    _result[1] = 1.0f / _a[1];
    _result[2] = 1.0f / _a[2];
...
void ct_vec3_tangent_frame(const float *_n,
                           float *_t,
                           float *_b) {
    const float nx = _n[0];
    const float ny = _n[1];
...
void ct_vec3_tangent_frame_a(const float *_n,
                             float *_t,
                             float *_b,
                             float _angle) {
...
void ct_vec3_from_lat_long(float *_vec,
                           float _u,
                           float _v) {
...
void ct_vec3_to_lat_long(float *_u,
                         float *_v,
                         const float *_vec) {
...
void ct_vec4_move(float *_result,
                  const float *_a) {
    _result[0] = _a[0];
    _result[1] = _a[1];
    _result[2] = _a[2];
...
void ct_quat_identity(float *_result) {
    _result[0] = 0.0f;
    _result[1] = 0.0f;
    _result[2] = 0.0f;
    _result[3] = 1.0f;
...
void ct_quat_move(float *_result,
                  const float *_a) {
    _result[0] = _a[0];
    _result[1] = _a[1];
    _result[2] = _a[2];
...
void ct_quat_mul_xyz(float *_result,
                     const float *_qa,
                     const float *_qb) {
...
void ct_quat_mul(float *_result,
                 const float *_qa,
                 const float *_qb) {
    const float ax = _qa[0];
    const float ay = _qa[1];
...
void ct_quat_invert(float *_result,
                    const float *_quat) {
    _result[0] = -_quat[0];
    _result[1] = -_quat[1];
    _result[2] = -_quat[2];
...
float ct_quat_dot(const float *_a,
                  const float *_b) {
    return _a[0] * _b[0]
           + _a[1] * _b[1]
           + _a[2] * _b[2]
...
void ct_quat_norm(float *_result,
                  const float *_quat) {
...
void ct_quat_to_euler(float *_result,
                      const float *_quat) {
...
void ct_quat_rotate_axis(float *_result,
                         const float *_axis,
                         float _angle) {
...
void ct_quat_rotate_x(float *_result,
                      float _ax) {
...
void ct_quat_rotate_y(float *_result,
                      float _ay) {
...
void ct_quat_rotate_z(float *_result,
                      float _az) {
...
void ct_vec3_mul_quat(float *_result,
                      const float *_vec,
                      const float *_quat) {
    float tmp0[4];
...
void ct_mat4_identity(float *_result) {
    memset(_result, 0, sizeof(float) * 16);
    _result[0] = _result[5] = _result[10] = _result[15] = 1.0f;
}

void ct_mat4_translate(float *_result,
                       float _tx,
                       float _ty,
                       float _tz) {
...
void ct_mat4_scale(float *_result,
                   float _sx,
                   float _sy,
                   float _sz) {
    memset(_result, 0, sizeof(float) * 16);
...
void ct_mat4_scale_s(float *_result,
                     float _scale) {
    ct_mat4_scale(_result, _scale, _scale, _scale);
}

void ct_mat4_from_normal(float *_result,
                         const float *_normal,
                         float _scale,
                         const float *_pos) {
    float tangent[3];
...
void ct_mat4_from_normal_a(float *_result,
                           const float *_normal,
                           float _scale,
                           const float *_pos,
                           float _angle) {
...
void ct_mat3_inverse(float *_result,
                     const float *_a) {
    float xx = _a[0];
    float xy = _a[1];
    float xz = _a[2];
...
void ct_mat4_inverse(float *_result,
                     const float *_a) {
    float xx = _a[0];
    float xy = _a[1];
    float xz = _a[2];
...
void ct_mat4_quat(float *_result,
                  const float *_quat) {
    const float x = _quat[0];
    const float y = _quat[1];
    const float z = _quat[2];
...
void ct_mat4_quat_translation(float *_result,
                              const float *_quat,
...
void ct_mat4_quat_translation_hmd(float *_result,
                                  const float *_quat,
                                  const float *_translation) {
    float quat[4];
    quat[0] = -_quat[0];
...
void ct_mat4_rotate_x(float *_result,
                      float _ax) {
...
void ct_mat4_rotate_y(float *_result,
                      float _ay) {
...
void ct_mat4_rotate_z(float *_result,
                      float _az) {
...
void ct_mat4_rotate_xy(float *_result,
                       float _ax,
                       float _ay) {
    const float sx = ct_fsin(_ax);
    const float cx = ct_fcos(_ax);
...
void ct_mat4_rotate_xyz(float *_result,
                        float _ax,
                        float _ay,
                        float _az) {
    const float sx = ct_fsin(_ax);
...
void ct_mat4_rotate_zyx(float *_result,
                        float _ax,
                        float _ay,
                        float _az) {
    const float sx = ct_fsin(_ax);
...
void ct_mat4_srt(float *_result,
                 float _sx,
                 float _sy,
                 float _sz,
                 float _ax,
...
void ct_vec3_mul_mtx(float *_result,
                     const float *_vec,
                     const float *_mat) {
...
void ct_vec3_mul_mat4_h(float *_result,
                        const float *_vec,
                        const float *_mat) {
...
void ct_vec4_mul_mtx(float *_result,
                     const float *_vec,
                     const float *_mat) {
    _result[0] = _vec[0] * _mat[0] + _vec[1] * _mat[4] + _vec[2] * _mat[8] +
                 _vec[3] * _mat[12];
...
void ct_mat4_mul(float *_result,
                 const float *_a,
                 const float *_b) {
...
void ct_mat4_transpose(float *_result,
                       const float *_a) {
    _result[0] = _a[0];
    _result[4] = _a[1];
    _result[8] = _a[2];
...
void ct_mat4_proj_flip_handedness(float *_dst,
                                  const float *_src) {
    _dst[0] = -_src[0];
    _dst[1] = -_src[1];
    _dst[2] = -_src[2];
...
void ct_mat4_view_flip_handedness(float *_dst,
                                  const float *_src) {
    _dst[0] = -_src[0];
    _dst[1] = _src[1];
    _dst[2] = -_src[2];
...
void ct_calc_normal(float *_result,
                    float *_va,
                    float *_vb,
                    float *_vc) {
    float ba[3];
...
void ct_calc_plane(float *_result,
                   float *_va,
                   float *_vb,
                   float *_vc) {
    float normal[3];
...
void ct_calc_linear_fit_2d(float *_result,
                           const void *_points,
                           uint32_t _stride,
                           uint32_t _numPoints) {
    float sumX = 0.0f;
...
void ct_calc_linear_fit_3d(float *_result,
                           const void *_points,
                           uint32_t _stride,
                           uint32_t _numPoints) {
    float sumX = 0.0f;
...
void ct_mat4_look_at_impl(float *_result,
                          const float *_eye,
                          const float *_view,
                          const float *_up) {
...
void ct_mat4_look_at_lh(float *_result,
                        const float *_eye,
                        const float *_at,
                        const float *_up) {
    float tmp[4];
...
void ct_mat4_look_at_rh(float *_result,
                        const float *_eye,
                        const float *_at,
                        const float *_up) {
    float tmp[4];
...
void ct_mat4_look_at(float *_result,
                     const float *_eye,
                     const float *_at,
                     const float *_up) {
...
void ct_mat4_ortho(float *_result,
                   float _left,
                   float _right,
                   float _bottom,
                   float _top,
...
void ct_mat4_ortho_lh(float *_result,
                      float _left,
                      float _right,
                      float _bottom,
                      float _top,
...
void ct_mat4_ortho_rh(float *_result,
                      float _left,
                      float _right,
                      float _bottom,
                      float _top,
...
void ct_mat4_proj_xywh(float *_result,
                       float _x,
                       float _y,
                       float _width,
                       float _height,
...
void ct_mat4_proj_impl(float *_result,
                       float _ut,
                       float _dt,
                       float _lt,
                       float _rt,
...
void ct_mat4_proj_impl_fovy(float *_result,
                            float _fovy,
                            float _aspect,
                            float _near,
                            float _far,
...
void ct_mat4_proj(float *_result,
                  float _ut,
                  float _dt,
                  float _lt,
                  float _rt,
...
void ct_mat4_proj_fov(float *_result,
                      const float *_fov,
                      float _near,
                      float _far,
                      bool _oglNdc) {
...
void ct_mat4_proj_fovy(float *_result,
                       float _fovy,
                       float _aspect,
                       float _near,
                       float _far,
...
void ct_mat4_proj_lh(float *_result,
                     float _ut,
                     float _dt,
                     float _lt,
                     float _rt,
...
void ct_mat4_proj_lh_fov(float *_result,
                         const float *_fov,
                         float _near,
                         float _far,
                         bool _oglNdc) {
...
void ct_mat4_proj_lh_fovy(float *_result,
                          float _fovy,
                          float _aspect,
                          float _near,
                          float _far,
...
void ct_mat4_proj_rh(float *_result,
                     float _ut,
                     float _dt,
                     float _lt,
                     float _rt,
...
void ct_mat4_proj_rh_fov(float *_result,
                         const float *_fov,
                         float _near,
                         float _far,
                         bool _oglNdc) {
...
void ct_mat4_proj_inf_xywh(float *_result,
                           float _x,
                           float _y,
                           float _width,
                           float _height,
...
void ct_mat4_proj_inf_iml_fov(float *_result,
                              const float *_fov,
                              float _near,
                              bool _oglNdc,
                              enum NearFar nearfar,
...
void ct_mat4_proj_inf_impl_fovy(float *_result,
                                float _fovy,
                                float _aspect,
                                float _near,
                                bool _oglNdc,
...
void ct_mat4_proj_inf_fov(float *_result,
                          const float *_fov,
                          float _near,
                          bool _oglNdc) {
...
void ct_mat4_proj_inf(float *_result,
                      float _ut,
                      float _dt,
                      float _lt,
                      float _rt,
...
void ct_mat4_proj_inf_fovy(float *_result,
                           float _fovy,
                           float _aspect,
                           float _near,
                           bool _oglNdc) {
...
void ct_mat4_proj_inf_lh(float *_result,
                         float _ut,
                         float _dt,
                         float _lt,
                         float _rt,
...
void ct_mat4_proj_inf_lh_fov(float *_result,
                             const float *_fov,
                             float _near,
                             bool _oglNdc) {
    ct_mat4_proj_inf_iml_fov(_result, _fov, _near,
...
void ct_mat4_proj_inf_lh_fovy(float *_result,
                              float _fovy,
                              float _aspect,
                              float _near,
                              bool _oglNdc) {
...
void ct_mat4_proj_inf_rh(float *_result,
                         float _ut,
                         float _dt,
                         float _lt,
                         float _rt,
...
void ct_mat4_proj_inf_rh_fov(float *_result,
                             const float *_fov,
                             float _near,
                             bool _oglNdc) {
    ct_mat4_proj_inf_iml_fov(_result, _fov, _near,
...
void ct_mat4_proj_inf_rh_fovy(float *_result,
                              float _fovy,
                              float _aspect,
                              float _near,
                              bool _oglNdc) {
...
void ct_mat4_proj_rev_inf_lh(float *_result,
                             float _ut,
                             float _dt,
                             float _lt,
                             float _rt,
...
void ct_mat4_proj_rev_inf_lh_fov(float *_result,
                                 const float *_fov,
                                 float _near,
                                 bool _oglNdc) {
    ct_mat4_proj_inf_iml_fov(_result, _fov, _near,
...
void ct_mat4_proj_rev_inf_lh_fovy(float *_result,
                                  float _fovy,
                                  float _aspect,
                                  float _near,
                                  bool _oglNdc) {
...
void ct_mat4_proj_rev_inf_rh(float *_result,
                             float _ut,
                             float _dt,
                             float _lt,
                             float _rt,
...
void ct_mat4_proj_rev_inf_rh_fov(float *_result,
                                 const float *_fov,
                                 float _near,
                                 bool _oglNdc) {
    ct_mat4_proj_inf_iml_fov(_result, _fov, _near,
...
void ct_mat4_proj_rev_inf_rh_fovy(float *_result,
                                  float _fovy,
                                  float _aspect,
                                  float _near,
                                  bool _oglNdc) {
