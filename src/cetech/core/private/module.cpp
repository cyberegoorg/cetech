...
static void load_dirs() {
    ...
    const char *path = ct_coredb_a0.read_string(_G.config_object,
                                                CONFIG_MODULE_DIR,
                                                "bin/darwin64");
    ...
    for (int i = 0; true; ++i) {
        ...
        const char *module_file = ct_coredb_a0.read_string(_G.config_object,
                                                           key_id, "");
        char *module_path = ct_path_a0.join(ct_memory_a0.main_allocator(),
                                            2, path, module_file);
        load(module_path);

        CT_FREE(ct_memory_a0.main_allocator(), module_path);
    }
}
