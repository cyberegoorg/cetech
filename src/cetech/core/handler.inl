#ifndef CECORE_HANDLERID_INL
#define CECORE_HANDLERID_INL
...
namespace celib {
    ...
    namespace handler {
        ...
        T create(Handler<T> &handler) {
            ...
            if (ct_array_size(handler._freeIdx) > _MINFREEINDEXS) {
                ...
                ct_array_push(handler._generation, 0, handler.alloc);
        ...
        void destroy(Handler<T> &handler,
            ...
            ct_array_push(handler._freeIdx, id, handler.alloc);
        }
    ...
    template<typename T>
    Handler<T>::Handler(ct_alloc *allocator) : _generation(nullptr),
                                               _freeIdx(nullptr),
                                               alloc(allocator) {
    }

    template<typename T>
    void Handler<T>::init(ct_alloc *allocator) {
        alloc = allocator;
        handler::create(*this);
    }
    ...
    void Handler<T>::destroy() {
        ct_array_free(_freeIdx, alloc);
        ct_array_free(_generation, alloc);
    }
};
