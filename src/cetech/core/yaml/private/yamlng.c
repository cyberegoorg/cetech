...
void add_key(const char *key,
    ...
    ct_array_push_n(_G.key_to_str_data, key, sizeof(char) * (key_len + 1),
                    _G.allocator);
...
uint32_t new_node(struct ct_yng_doc *doc,
    ...
    ct_array_push(inst->type, type, _G.allocator);
    ct_array_push(inst->hash, hash, _G.allocator);
    ct_array_push(inst->value, value, _G.allocator);
    ct_array_push(inst->first_child, 0, _G.allocator);
    ct_array_push(inst->next_sibling, 0, _G.allocator);
    ct_array_push(inst->parent, parent, _G.allocator);
...
struct ct_yamlng_node get_seq(ct_yng_doc_instance_t *_inst,
    ...
    return (struct ct_yamlng_node) {.idx = it, .d = inst->doc};
}
...
void set_float(ct_yng_doc_instance_t *_inst,
               struct ct_yamlng_node node,
               float value) {
    struct yamlng_document_inst *inst = (struct yamlng_document_inst *) _inst;
...
void foreach_dict_node(ct_yng_doc_instance_t *_inst,
    ...
    uint32_t it = inst->first_child[node.idx];
    while (0 != it) {
        struct ct_yamlng_node key = {.idx=it, .d = inst->doc};
        struct ct_yamlng_node value = {.idx=inst->first_child[it], .d = inst->doc};
...
void foreach_seq_node(ct_yng_doc_instance_t *_inst,
    ...
    uint32_t it = inst->first_child[node.idx];
    while (0 != it) {
        struct ct_yamlng_node value = {.idx=it, .d = inst->doc};
...
#define HAS_KEY() (parent_stack[parent_stack_top].type == NODE_STRING)
    do {
        parent_stack_top = ct_array_size(parent_stack) - 1;
        ...
        switch (event.type) {
            ...
            case YAML_SEQUENCE_START_EVENT:
                ...
                ct_hash_add(&inst->key_map, key, tmp_idx, _G.allocator);

                if (HAS_KEY()) {
                    ct_array_pop_back(parent_stack);
                }
                ...
                ct_array_push(parent_stack, state, _G.allocator);
                break;
...
//    unsigned char kim_nuke_shit[4096];
    ...
    struct ct_yamlng_node root_node = d->get(d->inst, 0);
...
struct ct_yamlng_node create_tree(ct_yng_doc_instance_t *_inst,
                                  const char **keys,
                                  uint32_t keys_count) {
    ...
    for (int i = first_nonexist; i < keys_count - 1; ++i) {
        ...
        ct_hash_add(&inst->key_map, key, new_map_idx, _G.allocator);
...
void create_tree_vec3(ct_yng_doc_instance_t *_inst,
    ...
    struct ct_yamlng_node node = create_tree(_inst, keys, keys_count);
    struct ct_yng_doc *d = node.d;
...
static void destroy(struct ct_yng_doc *document) {
    ...
    ct_array_free(inst->parent_file, _G.allocator);
    ct_array_free(inst->type, _G.allocator);
    ct_array_free(inst->hash, _G.allocator);
    ct_array_free(inst->value, _G.allocator);
    ct_array_free(inst->first_child, _G.allocator);
    ct_array_free(inst->next_sibling, _G.allocator);
    ct_array_free(inst->parent, _G.allocator);

    CT_FREE(alloc, inst);
    CT_FREE(alloc, document);
}

void parent_files(ct_yng_doc_instance_t *_inst,
                  const char ***files,
                  uint32_t *count) {
    struct yamlng_document_inst *inst = (struct yamlng_document_inst *) _inst;
...
static struct ct_yng_a0 yamlng_api = {
        .from_vio = from_vio,
...
static void _shutdown() {
    ct_hash_free(&_G.key_to_str, _G.allocator);
    ct_array_free(_G.key_to_str_offset, _G.allocator);
    ct_array_free(_G.key_to_str_data, _G.allocator);
