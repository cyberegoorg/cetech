...
void foreach_layer(const char *filename,
    ...
    if (ce_cdb_a0->prop_exist(root_key, ce_ydb_a0->key("render_state"))) {
        ...
        uint64_t render_state = ce_cdb_a0->read_subobject(root_key,
                                                          ce_ydb_a0->key(
                                                                  "render_state"),
                                                          0);
    ...
    if (ce_cdb_a0->prop_exist(root_key, ce_ydb_a0->key("variables"))) {
        uint64_t variables = ce_cdb_a0->read_subobject(root_key, ce_ydb_a0->key(
                "variables"), 0);
        ...
        for (uint32_t i = 0; i < variables_keys_count; ++i) {
            uint64_t _var = ce_cdb_a0->read_subobject(variables,
                                                      variables_keys[i], 0);
