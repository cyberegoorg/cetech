...
namespace ImGui {
    struct DockContext {
        ...
        struct Dock {
            Dock()
                    : id(0), label(NULL), next_tab(NULL), prev_tab(NULL),
                      parent(NULL), pos(0, 0), size(-1, -1), active(true),
                      status(Status_Float), opened(false), first(true) {
                location[0] = 0;
            ...
            void setChildrenPosSize(const ImVec2 &_pos,
                                    const ImVec2 &_size) {
                ImVec2 s = children[0]->size;
                if (isHorizontal()) {
                    s.y = _size.y;
                    s.x = (float) int(
                            _size.x * children[0]->size.x /
            ...
            void setPosSize(const ImVec2 &_pos,
                            const ImVec2 &_size) {
                size = _size;
                pos = _pos;
        ...
        DockContext()
                : m_current(NULL), m_last_frame(0) {
        }

        ~DockContext() {
        }

        Dock &getDock(const char *label,
                      bool opened) {
        ...
        void splits() {
            ...
            for (int i = 0; i < m_docks.size(); ++i) {
                ...
                if (dock.isHorizontal()) {
                    ...
                    dsize.x = -ImMin(-dsize.x,
                                     dock.children[0]->size.x - min_size0.x);
                    dsize.x = ImMin(dsize.x,
                                    dock.children[1]->size.x - min_size1.x);
                } else {
                    InvisibleButton("split", ImVec2(dock.size.x, 3));
                    if (dock.status == Status_Dragged)
                        dsize.y = io.MouseDelta.y;
                    dsize.y = -ImMin(-dsize.y,
                                     dock.children[0]->size.y - min_size0.y);
                    dsize.y = ImMin(dsize.y,
                                    dock.children[1]->size.y - min_size1.y);
                }
        ...
        void checkNonexistent() {
            ...
            for (DockVector::iterator it = m_docks.begin(), itEnd = m_docks.end();
                 it != itEnd; ++it) {
                Dock *dock = *it;
        ...
        void beginPanel() {
            ...
            if (root) {
                const ImVec2 percentage(displaySize.x / root->size.x,
                                        displaySize.y / root->size.y);
        ...
        Dock *getDockAt(const ImVec2 & /*pos*/) const {
            for (int i = 0; i < m_docks.size(); ++i) {
                ...
                if (IsMouseHoveringRect(dock.pos, dock.pos + dock.size,
                                        false)) {
                    return &dock;
                }
            }
        ...
        static ImRect getDockedRect(const ImRect &rect,
                                    Slot_ dock_slot) {
            ...
            switch (dock_slot) {
                default:
                    return rect;
                case Slot_Top:
                    return ImRect(rect.Min,
                                  rect.Min + ImVec2(rect.Max.x, half_size.y));
                case Slot_Right:
                ...
                case Slot_Left:
                    return ImRect(rect.Min,
                                  rect.Min + ImVec2(half_size.x, rect.Max.y));
            }
        }

        static ImRect getSlotRect(ImRect parent_rect,
                                  Slot_ dock_slot) {
            ImVec2 size = parent_rect.Max - parent_rect.Min;
            ImVec2 center = parent_rect.Min + size * 0.5f;
            switch (dock_slot) {
                default:
                    return ImRect(center - ImVec2(20, 20),
                                  center + ImVec2(20, 20));
                case Slot_Top:
                    return ImRect(center + ImVec2(-20, -50),
                                  center + ImVec2(20, -30));
                case Slot_Right:
                    return ImRect(center + ImVec2(30, -20),
                                  center + ImVec2(50, 20));
                case Slot_Bottom:
                    return ImRect(center + ImVec2(-20, +30),
                                  center + ImVec2(20, 50));
                case Slot_Left:
                    return ImRect(center + ImVec2(-50, -20),
                                  center + ImVec2(-30, 20));
            }
        }
        ...
        Dock *getRootDock() {
            for (int i = 0; i < m_docks.size(); ++i) {
                if (!m_docks[i]->parent &&
                    (m_docks[i]->status == Status_Docked ||
                     m_docks[i]->children[0])) {
                    return m_docks[i];
                }
            }
            return NULL;
        ...
        bool dockSlots(Dock &dock,
            ...
            for (int i = 0; i < (on_border ? 4 : 5); ++i) {
                ImRect r =
                        on_border ? getSlotRectOnBorder(rect, (Slot_) i)
                                  : getSlotRect(rect, (Slot_) i);
                bool hovered = r.Contains(mouse_pos);

                canvas->AddRectFilled(r.Min, r.Max,
                                      hovered ? color_hovered : color);
                if (!hovered) continue;

                if (!IsMouseDown(0)) {
                    doDock(dock, dest_dock ? dest_dock : getRootDock(),
                           (Slot_) i);
                    return true;
                }
        ...
        void handleDrag(Dock &dock) {
            ...
            if (dest_dock) {
                if (dockSlots(dock,
                              dest_dock,
                              ImRect(dest_dock->pos,
                                     dest_dock->pos + dest_dock->size),
                              false)) {
                    canvas->PopClipRect();
                    End();
                    return;
            ...
            if (dockSlots(dock, NULL, ImRect(ImVec2(0, 0), GetIO().DisplaySize),
                          true)) {
                canvas->PopClipRect();
                End();
                return;
        ...
        void doUndock(Dock &dock) {
            ...
            if (container) {
                ...
                if (remove_container) {
                    if (container->parent) {
                        Dock *&child =
                                container->parent->children[0] == container
                        ...
                        if (container->children[0]) {
                            ...
                            container->children[0]->setPosSize(container->pos,
                                                               container->size);
                        }
                        if (container->children[1]) {
                            container->children[1]->setParent(NULL);
                            container->children[1]->setPosSize(container->pos,
                                                               container->size);
                        }
                    }
                    for (int i = 0; i < m_docks.size(); ++i) {
        ...
        bool tabbar(Dock &dock,
            ...
            if (BeginChild(tmp, size0, true)) {
                ...
                while (dock_tab) {
                    ...
                    ImVec2 size1(CalcTextSize(dock_tab->label, text_end).x,
                                 line_height);
                    ...
                    if (dock_tab->active && close_button) {
                        ...
                        ImVec2 center = ((GetItemRectMin() + GetItemRectMax()) *
                                         0.5f);
        ...
        void rootDock(const ImVec2 &pos,
                      const ImVec2 &size) {
            Dock *root = getRootDock();
            if (!root) return;
        ...
        void tryDockToStoredLocation(Dock &dock) {
            ...
            doDock(dock, tmp ? tmp : prev, tmp && !tmp->children[0] ? Slot_Tab
                                                                    : getSlotFromLocationCode(
                            *c));
        }
        ...
        bool begin(const char *label,
            ...
            if (is_float) {
                ...
                if (g.ActiveId == GetCurrentWindow()->MoveId &&
                    g.IO.MouseDown[0]) {
        ...
        void saveToYaml(char **buffer,
            ...
            for (int i = 0; i < m_docks.size(); ++i) {
                ...
                const char *label =
                        strlen(dock.label) > 0 ? dock.label : "\"\"";
                const char *location =
                        strlen(dock.location) > 0 ? dock.location : "\"\"";
        ...
        void loadFromYaml(const char *path,
            ...
            for (uint32_t i = 0; i < type_keys_count; ++i) {
                ...
                const char *label_n = cdb->read_str(d_reader, yng->key("label"),
                                                    "");
                const char *location_n = cdb->read_str(d_reader,
                                                       yng->key("location"),
                                                       "");

                int x_n = (int) cdb->read_float(d_reader, yng->key("x"), 0);
                int y_n = (int) cdb->read_float(d_reader, yng->key("y"), 0);
                int size_x_n = (int) cdb->read_float(d_reader,
                                                     yng->key("size_x"), 0);
                int size_y_n = (int) cdb->read_float(d_reader,
                                                     yng->key("size_y"), 0);
                int status_n = (int) cdb->read_float(d_reader,
                                                     yng->key("status"), 0);
                int active_n = (int) cdb->read_float(d_reader,
                                                     yng->key("active"), 0);
                int opened_n = (int) cdb->read_float(d_reader,
                                                     yng->key("opened"), 0);
                int prev_n = (int) cdb->read_float(d_reader, yng->key("prev"),
                                                   0);

                int next_n = (int) cdb->read_float(d_reader, yng->key("next"),
                                                   0);

                int child0_n = (int) cdb->read_float(d_reader,
                                                     yng->key("child0"), 0);
                int child1_n = (int) cdb->read_float(d_reader,
                                                     yng->key("child1"), 0);
                int parent_n = (int) cdb->read_float(d_reader,
                                                     yng->key("parent"), 0);
