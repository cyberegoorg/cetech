...
void ImGui::ShowTestWindow(bool *p_open) {
    ...
    ImGui::PushItemWidth(
            -140);                                 // Right align, keep 140 pixels for labels
    ...
    if (ImGui::CollapsingHeader("Widgets")) {
            ...
            if (ImGui::TreeNode("Advanced, with Selectable nodes")) {
                ...
                if (node_clicked != -1) {
                    ...
                    if (ImGui::GetIO().KeyCtrl)
                        selection_mask ^= (1
                                << node_clicked);          // CTRL+click to toggle
                    else //if (!(selection_mask & (1 << node_clicked))) // Depending on selection behavior you want, this commented bit preserve selection when clicking on item that is part of the selection
                        selection_mask = (1
                                << node_clicked);           // Click to single-select
                }
                ImGui::PopStyleVar();
                if (align_label_with_current_x_position)
        ...
        if (ImGui::TreeNode("Selectables")) {
            ...
            if (ImGui::TreeNode("Grid")) {
                static bool selected[16] = {true, false, false, false, false,
        ...
        const char *listbox_items[] = {"Apple", "Banana", "Cherry", "Kiwi",
        ...
        if (ImGui::TreeNode("Vertical Sliders")) {
            ...
            static float values[7] = {0.0f, 0.60f, 0.35f, 0.9f, 0.70f, 0.20f,
                                      0.0f};
            ImGui::PushID("set1");
            for (int i = 0; i < 7; i++) {
                if (i > 0) ImGui::SameLine();
    ...
    if (ImGui::CollapsingHeader("Graphs widgets")) {
        ...
        struct Funcs {
            static float Sin(void *,
                             int i) { return sinf(i * 0.1f); }

            static float Saw(void *,
                             int i) { return (i & 1) ? 1.0f : 0.0f; }
        };
        ...
        float (*func)(void *,
                      int) = (func_type == 0) ? Funcs::Sin : Funcs::Saw;
        ...
        float progress_saturated = (progress < 0.0f) ? 0.0f : (progress > 1.0f)
                                                              ? 1.0f : progress;
        char buf[32];
    ...
    if (ImGui::CollapsingHeader("Layout")) {
        ...
        if (ImGui::TreeNode("Groups")) {
            ...
            ImGui::Button("ACTION",
                          ImVec2((size.x - ImGui::GetStyle().ItemSpacing.x) *
                                 0.5f, size.y));
            ImGui::SameLine();
        ...
        if (ImGui::TreeNode("Horizontal Scrolling")) {
            ...
            ImGui::BeginChild("scrolling", ImVec2(0,
            ...
            for (int line = 0; line < lines; line++) {
                ...
                for (int n = 0; n < num_buttons; n++) {
                    ...
                    char num_buf[16];
                    const char *label = (!(n % 15)) ? "FizzBuzz" : (!(n % 3))
                                                                   ? "Fizz"
                                                                   : (!(n % 5))
                                                                     ? "Buzz"
                                                                     : (sprintf(
                    ...
                    float hue = n * 0.05f;
                    ImGui::PushStyleColor(ImGuiCol_Button,
                                          ImColor::HSV(hue, 0.6f, 0.6f));
                    ImGui::PushStyleColor(ImGuiCol_ButtonHovered,
                                          ImColor::HSV(hue, 0.7f, 0.7f));
                    ImGui::PushStyleColor(ImGuiCol_ButtonActive,
                                          ImColor::HSV(hue, 0.8f, 0.8f));
                    ImGui::Button(label, ImVec2(40.0f +
                                                sinf((float) (line + n)) *
                                                20.0f, 0.0f));
                    ImGui::PopStyleColor(3);
                    ImGui::PopID();
    ...
    if (ImGui::CollapsingHeader("Popups & Modal windows")) {
        ...
        if (ImGui::TreeNode("Modals")) {
            ...
            if (ImGui::BeginPopupModal("Delete?", NULL,
                ...
                if (ImGui::Button("OK", ImVec2(120,
                                               0))) { ImGui::CloseCurrentPopup(); }
                ImGui::SameLine();
                if (ImGui::Button("Cancel", ImVec2(120,
                                                   0))) { ImGui::CloseCurrentPopup(); }
                ImGui::EndPopup();
            }
    ...
    if (ImGui::CollapsingHeader("Keyboard, Mouse & Focus")) {
        ...
        if (ImGui::TreeNode("Dragging")) {
            ...
            if (ImGui::IsItemActive()) {
                ...
                draw_list->AddLine(
                        ImGui::CalcItemRectClosestPoint(ImGui::GetIO().MousePos,
                                                        true, -2.0f),
                ...
                ImGui::Text(
                        ...
                        value_with_lock_threshold.y, mouse_delta.x,
                        mouse_delta.y);
            }
            ImGui::TreePop();
        }
...
void ImGui::ShowStyleEditor(ImGuiStyle *ref) {
    ...
    if (ImGui::TreeNode("Colors")) {
        ...
        if (ImGui::Button("Copy Colors")) {
            ...
            for (int i = 0; i < ImGuiCol_COUNT; i++) {
                ...
                if (!output_only_modified || memcmp(&col, (ref ? &ref->Colors[i]
                                                               : &default_style.Colors[i]),
                                                    sizeof(ImVec4)) != 0)
                    ImGui::LogText(
                            "style.Colors[ImGuiCol_%s]%*s= ImVec4(%.2ff, %.2ff, %.2ff, %.2ff);" IM_NEWLINE,
        ...
        for (int i = 0; i < ImGuiCol_COUNT; i++) {
            ...
            if (memcmp(&style.Colors[i],
                ...
                if (ImGui::Button("Revert"))
                    style.Colors[i] = ref ? ref->Colors[i]
                                          : default_style.Colors[i];
                if (ref) {
                    ImGui::SameLine();
    ...
    if (ImGui::TreeNode("Fonts", "Fonts (%d)",
        ...
        if (ImGui::TreeNode("Atlas texture", "Atlas texture (%dx%d pixels)",
            ...
            ImGui::Image(atlas->TexID, ImVec2((float) atlas->TexWidth,
        ...
        for (int i = 0; i < atlas->Fonts.Size; i++) {
            ...
            if (ImGui::TreeNode("Details")) {
                ...
                ImGui::Text("Texture surface: %d pixels (approx) ~ %dx%d",
                            ...
                            (int) sqrtf((float) font->MetricsTotalSurface));
                for (int config_i = 0;
                     config_i < font->ConfigDataCount; config_i++) {
                ...
                if (ImGui::TreeNode("Glyphs", "Glyphs (%d)",
                    ...
                    for (int base = 0; base < 0x10000; base += 256) {
                        int count = 0;
                        for (int n = 0; n < 256; n++)
                            count += font->FindGlyph((ImWchar) (base + n)) ? 1
                                                                           : 0;
                        if (count > 0 &&
                            ...
                            ImDrawList *draw_list = ImGui::GetWindowDrawList();
                            for (int n = 0; n < 256; n++) {
                                ImVec2 cell_p1(base_pos.x + (n % 16) *
                                                            (cell_size.x +
                                                             cell_spacing),
                                               base_pos.y + (n / 16) *
                                                            (cell_size.y +
                                ...
                                const ImFont::Glyph *glyph = font->FindGlyph(
                                        (ImWchar) (base + n));;
                                draw_list->AddRect(cell_p1, cell_p2,
                                                   glyph ? IM_COL32(255, 255,
                                                                    255, 100)
                                                         : IM_COL32(255, 255,
                                                                    255, 50));
                                font->RenderChar(draw_list, cell_size.x,
                                                 cell_p1, ImGui::GetColorU32(
                                                ImGuiCol_Text),
                                                 (ImWchar) (base +
                                                            n)); // We use ImFont::RenderChar as a shortcut because we don't have UTF-8 conversion functions available to generate a string.
                                if (glyph && ImGui::IsMouseHoveringRect(cell_p1,
                                                                        cell_p2)) {
                                    ImGui::BeginTooltip();
...
static void ShowExampleAppConstrainedResize(bool *p_open) {
    ...
    static int type = 0;
    if (type == 0)
        ImGui::SetNextWindowSizeConstraints(ImVec2(-1, 0), ImVec2(-1,
                                                                  FLT_MAX));      // Vertical only
    if (type == 1)
        ImGui::SetNextWindowSizeConstraints(ImVec2(0, -1), ImVec2(FLT_MAX,
                                                                  -1));      // Horizontal only
    if (type == 2)
        ImGui::SetNextWindowSizeConstraints(ImVec2(100, 100), ImVec2(FLT_MAX,
                                                                     FLT_MAX)); // Width > 100, Height > 100
    if (type == 3)
        ImGui::SetNextWindowSizeConstraints(ImVec2(300, 0), ImVec2(400,
                                                                   FLT_MAX));     // Width 300-400
    if (type == 4)
...
struct ExampleAppConsole {
    ...
    // Portable helpers
    static int Stricmp(const char *str1,
                       const char *str2) {
        int d;
    ...
    static int Strnicmp(const char *str1,
                        const char *str2,
                        int n) {
        int d = 0;
    ...
    void Draw(const char *title,
        ...
        if (ImGui::InputText("Input", InputBuf, IM_ARRAYSIZE(InputBuf),
            ...
            while (input_end > InputBuf &&
                   input_end[-1] == ' ') { input_end--; }
            *input_end = 0;
            if (InputBuf[0])
    ...
    static int
    TextEditCallbackStub(ImGuiTextEditCallbackData *data) // In C++11 you are better off using lambdas for this sort of forwarding callbacks
    {
...
static void ShowExampleAppLayout(bool *p_open) {
    ...
    if (ImGui::Begin("Example: Layout", p_open, ImGuiWindowFlags_MenuBar)) {
        ...
        ImGui::BeginChild("item view", ImVec2(0,
                                              -ImGui::GetItemsLineHeightWithSpacing())); // Leave room for 1 line below us
...
static void ShowExampleAppPropertyEditor(bool *p_open) {
    ...
    struct funcs {
        static void ShowDummyObject(const char *prefix,
                                    int uid) {
            ImGui::PushID(
            ...
            ImGui::NextColumn();
            if (node_open) {
                static float dummy_members[8] = {0.0f, 0.0f, 1.0f, 3.1416f,
                                                 100.0f, 999.0f};
                for (int i = 0; i < 8; i++) {
