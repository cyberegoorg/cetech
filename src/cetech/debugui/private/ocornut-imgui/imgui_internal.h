...
static inline ImVec2 &operator+=(ImVec2 &lhs,
                                 const ImVec2 &rhs) {
    lhs.x += rhs.x;
    lhs.y += rhs.y;
    return lhs;
...
static inline int ImMin(int lhs,
                        int rhs) { return lhs < rhs ? lhs : rhs; }

static inline int ImMax(int lhs,
                        int rhs) { return lhs >= rhs ? lhs : rhs; }

static inline float ImMin(float lhs,
                          float rhs) { return lhs < rhs ? lhs : rhs; }

static inline float ImMax(float lhs,
                          float rhs) { return lhs >= rhs ? lhs : rhs; }

static inline ImVec2 ImMin(const ImVec2 &lhs,
                           const ImVec2 &rhs) {
    return ImVec2(ImMin(lhs.x, rhs.x), ImMin(lhs.y, rhs.y));
}

static inline ImVec2 ImMax(const ImVec2 &lhs,
                           const ImVec2 &rhs) {
    return ImVec2(ImMax(lhs.x, rhs.x), ImMax(lhs.y, rhs.y));
}

static inline int ImClamp(int v,
                          int mn,
                          int mx) { return (v < mn) ? mn : (v > mx) ? mx : v; }

static inline float ImClamp(float v,
                            float mn,
                            float mx) {
    return (v < mn) ? mn : (v > mx) ? mx : v;
}

static inline ImVec2 ImClamp(const ImVec2 &f,
                             const ImVec2 &mn,
                             ImVec2 mx) {
...
static inline float ImLerp(float a,
                           float b,
                           float t) { return a + (b - a) * t; }
...
static inline float ImInvLength(const ImVec2 &lhs,
                                float fail_value) {
...
enum ImGuiButtonFlags_ {
    ...
    ImGuiButtonFlags_AllowOverlapMode = 1
...
struct IMGUI_API ImRect {
    ...
    ImVec2 GetClosestPoint(ImVec2 p,
                           bool on_edge) const {
...
struct ImGuiStyleMod {
    ...
    ImGuiStyleMod(ImGuiStyleVar idx,
                  int v) {
        VarIdx = idx;
        BackupInt[0] = v;
    }
...
struct IMGUI_API ImGuiTextEditState {
    ...
    void
    ClearSelection() { StbState.select_start = StbState.select_end = StbState.cursor; }
...
struct IMGUI_API ImGuiDrawContext {
    ...
    ImGuiDrawContext() {
        CursorPos = CursorPosPrevLine = CursorStartPos = CursorMaxPos = ImVec2(
                0.0f, 0.0f);
...
public:
    ...
    float MenuBarHeight() const {
        return (Flags & ImGuiWindowFlags_MenuBar) ? CalcFontSize() +
                                                    GImGui->Style.FramePadding.y *
                                                    2.0f : 0.0f;
    }
...
namespace ImGui {
    ...
    IMGUI_API void ItemSize(const ImVec2 &size,
                            float text_offset_y = 0.0f);

    IMGUI_API void ItemSize(const ImRect &bb,
                            float text_offset_y = 0.0f);
    ...
    IMGUI_API bool IsHovered(const ImRect &bb,
                             ImGuiID id,
    ...
    IMGUI_API ImVec2 CalcItemSize(ImVec2 size,
                                  float default_x,
                                  float default_y);

    IMGUI_API float CalcWrapWidthForPos(const ImVec2 &pos,
                                        float wrap_pos_x);

    IMGUI_API void OpenPopupEx(const char *str_id,
    ...
    IMGUI_API void RenderCollapseTriangle(ImVec2 pos,
                                          bool is_open,
                                          float scale = 1.0f);

    IMGUI_API void RenderBullet(ImVec2 pos);

    IMGUI_API void RenderCheckMark(ImVec2 pos,
                                   ImU32 col);
    ...
    IMGUI_API bool CloseButton(ImGuiID id,
                               const ImVec2 &pos,
                               float radius);
    ...
    IMGUI_API int ParseFormatPrecision(const char *fmt,
                                       int default_value);

    IMGUI_API float RoundScalar(float value,
                                int decimal_precision);
