...
#ifndef CECORE_ARRAY2_INL_H
#define CECORE_ARRAY2_INL_H
...
namespace celib {
    ...
    namespace array {
        ...
        void set_capacity(Array <T> &a,
            ...
            T *new_data = 0;
            if (new_capacity > 0) {
                new_data = CT_ALLOC(a._allocator, T,
                                       sizeof(T) * new_capacity);
                memcpy(new_data, a._data, sizeof(T) * a._size);
            }

            CT_FREE(a._allocator, a._data);
    ...
    template<typename T>
    inline
    Array<T>::Array(struct ct_alloc *a) : _allocator(a), _size(0),
                                          _capacity(0), _data(0) {}

    template<typename T>
    inline
    void Array<T>::init(ct_alloc *a) {
        _data = nullptr;
        _allocator = a;
        _size = 0;
        _capacity = 0;
