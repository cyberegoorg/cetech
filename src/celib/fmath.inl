...
static inline void ce_vec3_move(float *_result,
                                const float *_a) {
    _result[0] = _a[0];
    _result[1] = _a[1];
    _result[2] = _a[2];
...
static inline void ce_vec3_abs(float *_result,
                               const float *_a) {
    _result[0] = ce_fabsolute(_a[0]);
    _result[1] = ce_fabsolute(_a[1]);
    _result[2] = ce_fabsolute(_a[2]);
}

static inline void ce_vec3_neg(float *_result,
                               const float *_a) {
    _result[0] = -_a[0];
    _result[1] = -_a[1];
    _result[2] = -_a[2];
...
static inline void ce_vec3_add(float *_result,
                               const float *_a,
                               const float *_b) {
    _result[0] = _a[0] + _b[0];
    _result[1] = _a[1] + _b[1];
...
static inline void ce_vec3_add_s(float *_result,
                                 const float *_a,
                                 float _b) {
    _result[0] = _a[0] + _b;
    _result[1] = _a[1] + _b;
...
static inline void ce_vec3_sub(float *_result,
                               const float *_a,
                               const float *_b) {
    _result[0] = _a[0] - _b[0];
    _result[1] = _a[1] - _b[1];
...
static inline void ce_vec3_sub_s(float *_result,
                                 const float *_a,
                                 float _b) {
    _result[0] = _a[0] - _b;
    _result[1] = _a[1] - _b;
...
static inline void ce_vec3_mul(float *_result,
                               const float *_a,
                               const float *_b) {
    _result[0] = _a[0] * _b[0];
    _result[1] = _a[1] * _b[1];
...
static inline void ce_vec3_mul_s(float *_result,
                                 const float *_a,
                                 float _b) {
    _result[0] = _a[0] * _b;
    _result[1] = _a[1] * _b;
...
static inline float ce_vec3_dot(const float *_a,
                                const float *_b) {
    return _a[0] * _b[0] + _a[1] * _b[1] + _a[2] * _b[2];
}

static inline void ce_vec3_cross(float *_result,
                                 const float *_a,
                                 const float *_b) {
...
static inline float ce_vec3_length(const float *_a) {
    return ce_fsqrt(ce_vec3_dot(_a, _a));
}

static inline void ce_vec3_lerp_s(float *_result,
                                  const float *_a,
                                  const float *_b,
                                  float _t) {
    _result[0] = ce_flerp(_a[0], _b[0], _t);
    _result[1] = ce_flerp(_a[1], _b[1], _t);
    _result[2] = ce_flerp(_a[2], _b[2], _t);
}

static inline void ce_vec3_lerp(float *_result,
                                const float *_a,
                                const float *_b,
                                const float *_c) {
    _result[0] = ce_flerp(_a[0], _b[0], _c[0]);
    _result[1] = ce_flerp(_a[1], _b[1], _c[1]);
    _result[2] = ce_flerp(_a[2], _b[2], _c[2]);
}

static inline float ce_vec3_norm(float *_result,
                                 const float *_a) {
...
static inline void ce_vec3_min(float *_result,
                               const float *_a,
                               const float *_b) {
...
static inline void ce_vec3_max(float *_result,
                               const float *_a,
                               const float *_b) {
...
static inline void ce_vec3_rcp(float *_result,
                               const float *_a) {
    _result[0] = 1.0f / _a[0];
    _result[1] = 1.0f / _a[1];
    _result[2] = 1.0f / _a[2];
...
static inline void ce_vec3_tangent_frame(const float *_n,
                                         float *_t,
                                         float *_b) {
    const float nx = _n[0];
    const float ny = _n[1];
    ...
    ce_vec3_cross(_b, _n, _t);
}

static inline void ce_vec3_tangent_frame_a(const float *_n,
                                           float *_t,
                                           float *_b,
                                           float _angle) {
...
static inline void ce_vec3_from_lat_long(float *_vec,
                                         float _u,
                                         float _v) {
...
static inline void ce_vec3_to_lat_long(float *_u,
                                       float *_v,
                                       const float *_vec) {
    const float phi = ce_fatan2(_vec[0], _vec[2]);
    const float theta = ce_facos(_vec[1]);
...
static inline void ce_quat_identity(float *_result) {
    _result[0] = 0.0f;
    _result[1] = 0.0f;
    _result[2] = 0.0f;
    _result[3] = 1.0f;
...
static inline void ce_quat_move(float *_result,
                                const float *_a) {
    _result[0] = _a[0];
    _result[1] = _a[1];
    _result[2] = _a[2];
...
static inline void ce_quat_mul_xyz(float *_result,
                                   const float *_qa,
                                   const float *_qb) {
...
static inline void ce_quat_mul(float *_result,
                               const float *_qa,
                               const float *_qb) {
    const float ax = _qa[0];
    const float ay = _qa[1];
...
static inline void ce_quat_invert(float *_result,
                                  const float *_quat) {
    _result[0] = -_quat[0];
    _result[1] = -_quat[1];
    _result[2] = -_quat[2];
...
static inline float ce_quat_dot(const float *_a,
                                const float *_b) {
    return _a[0] * _b[0]
           + _a[1] * _b[1]
           + _a[2] * _b[2]
...
static inline void ce_quat_norm(float *_result,
                                const float *_quat) {
    ...
    if (0.0f < norm) {
        ...
        ce_quat_identity(_result);
    }
}

static inline void ce_quat_to_euler(float *_result,
                                    const float *_quat) {
    ...
    _result[0] = ce_fatan2(2.0f * (x * w - y * z),
                           1.0f - 2.0f * (xx + zz));
    _result[1] = ce_fatan2(2.0f * (y * w + x * z),
                           1.0f - 2.0f * (yy + zz));
    _result[2] = ce_fasin(2.0f * (x * y + z * w));
}

static inline void ce_quat_from_euler(float *result,
                                      float heading,
                                      float attitude,
                                      float bank) {
...
static inline void ce_vec3_mul_quat(float *_result,
                                    const float *_vec,
                                    const float *_quat) {
    float tmp0[4];
    ce_quat_invert(tmp0, _quat);
    ...
    float tmp1[4];
    ce_quat_mul(tmp1, tmp0, qv);

    ce_quat_mul_xyz(_result, tmp1, _quat);
}

// # Vec4

static inline void ce_vec4_move(float *_result,
                                const float *_a) {
    _result[0] = _a[0];
    _result[1] = _a[1];
    _result[2] = _a[2];
...
static inline void ce_mat4_from_normal(float *_result,
    ...
    ce_vec3_mul_s(&_result[0], bitangent, _scale);
    ce_vec3_mul_s(&_result[4], _normal, _scale);
    ce_vec3_mul_s(&_result[8], tangent, _scale);
...
static inline void ce_calc_normal(float *_result,
                                  float *_va,
                                  float *_vb,
                                  float *_vc) {
    float ba[3];
    ce_vec3_sub(ba, _vb, _va);

    float ca[3];
    ce_vec3_sub(ca, _vc, _va);

    float baxca[3];
    ce_vec3_cross(baxca, ba, ca);

    ce_vec3_norm(_result, baxca);
}

static inline void ce_calc_plane(float *_result,
                                 float *_va,
                                 float *_vb,
                                 float *_vc) {
    float normal[3];
    ce_calc_normal(normal, _va, _vb, _vc);

    _result[0] = normal[0];
    _result[1] = normal[1];
    _result[2] = normal[2];
    _result[3] = -ce_vec3_dot(normal, _va);
}
...
static inline void ce_mat4_look_at_impl(float *_result,
    ...
    float tmp[4];
    ce_vec3_cross(tmp, up, _view);

    float right[4];
    ce_vec3_norm(right, tmp);

    ce_vec3_cross(up, _view, right);
...
static inline void ce_mat4_look_at_lh(float *_result,
                                      ...
                                      const float *_up) {
    float tmp[4];
    ce_vec3_sub(tmp, _at, _eye);

    float view[4];
    ce_vec3_norm(view, tmp);
