...
void copy_file(struct ce_alloc *allocator,
               const char *from,
               const char *to) {
    struct ce_vio *source_vio = ce_os_a0->vio->from_file(from,
                                                            VIO_OPEN_READ);
    ...
    struct ce_vio *build_vio = ce_os_a0->vio->from_file(to, VIO_OPEN_WRITE);
...
struct ce_os_path_a0 *ct_path_a0 = &path_api;
