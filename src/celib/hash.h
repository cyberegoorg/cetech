...
static void cel_hash_remove(struct cel_hash_t *hash,
    ...
    const uint32_t last_idx = --hash->n;

    hash->keys[idx] = hash->keys[last_idx];
    cel_array_pop_back(hash->keys);

    hash->values[idx] = hash->values[last_idx];
    cel_array_pop_back(hash->values);
}
