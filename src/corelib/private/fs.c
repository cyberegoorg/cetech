...
static void map_root_dir(uint64_t root,
    ...
    if (watch) {
//        struct ct_watchdog *wd = ct_os_a0->watchdog->create(_G.allocator);
...
static bool exist_dir(const char *full_path) {
    ...
    ct_os_a0->path->dir(path_buffer, full_path);
    return ct_os_a0->path->is_dir(path_buffer);
}

static bool exist(const char *full_path) {
    struct ct_vio *f = ct_os_a0->vio->from_file(full_path, VIO_OPEN_READ);
    if (f != NULL) {
        f->close(f);
        return true;
    }
...
static char *get_full_path(uint64_t root,
    ...
    for (uint32_t i = 0; i < mp_count; ++i) {
        ...
        char *fullpath = NULL;
        ct_os_a0->path->join(&fullpath, allocator, 2, mp->root_path,
                             filename);
...
static struct ct_vio *open(uint64_t root,
    ...
    struct ct_vio *file = ct_os_a0->vio->from_file(full_path,
...
static int create_directory(uint64_t root,
    ...
    int ret = ct_os_a0->path->make_path(full_path);
...
static void listdir(uint64_t root,
    ...
    for (uint32_t i = 0; i < mp_count; ++i) {
        ...
        ct_os_a0->path->join(&final_path, allocator, 2, mount_point_dir,
                             path);
        ct_os_a0->path->list(final_path, (const char *[]) {filter}, 1,
        ...
        ct_os_a0->path->list_free(_files, _count, allocator);
...
static int64_t get_file_mtime(uint64_t root,
    ...
    time_t ret = ct_os_a0->path->file_mtime(full_path);
