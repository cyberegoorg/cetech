...
void scratch_allocator_ctor(struct allocator_scratch *a, struct allocator *backing, uint32_t size) {
    ...
    a->backing = backing;
    a->begin = CE_ALLOCATE(backing, char, size);
    a->end = a->begin + size;
    a->allocate = a->begin;
    a->free = a->begin;
}

void scratch_allocator_dtor(struct allocator_scratch *a) {
    allocator_deallocate(a->backing, a->begin);
}
