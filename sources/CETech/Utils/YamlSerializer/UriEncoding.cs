...
namespace System.Yaml
{
    ...
    {
        private static readonly Regex NonUriChar = new Regex(@"[^0-9A-Za-z\-_.!~*'()\\;/?:@&=$,\[\]]");
        private static readonly Regex NonTagChar = new Regex(@"[^0-9A-Za-z\-_.!~*'()\\;/?:@&=$]");
            ...
            return NonUriChar.Replace(s, m =>
            {
                ...
                return c == ' '
                    ...
                    : c < 0x80
                        ? IntToHex(c)
                        : c < 0x0800
                            ? IntToHex(((c >> 6) & 0x1f) + 0xc0, (c & 0x3f) + 0x80)
        ...
        private static string IntToHex(int c)
        {
        ...
        private static string IntToHex(int c1, int c2)
        {
            return new string(new[]
            {
        ...
        private static string IntToHex(int c1, int c2, int c3)
        {
            return new string(new[]
            {
                '%', intToHex[c1 >> 4], intToHex[c1 & 0x0f],
