...
namespace CETech
{
    ...
    public class ShaderResource
    {
        ...
        private static void shaderc(string input, string output, string include_path, string type, string platform, string profile)
        {
            // Prepare the process to run
            ProcessStartInfo start = new ProcessStartInfo();
            // Enter in the command line arguments, everything you would enter after the executable name itself
            start.Arguments = string.Format("-f {0} -o {1} -i {2} --type {3} --platform {4} --profile {5}", input, output, include_path, type, platform, profile);
