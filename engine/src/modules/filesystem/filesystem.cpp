...
namespace filesystem {
    void map_root_dir(uint64_t root,
        ...
        map::set(_G.root_map, root,
                 ct_memory_api_v0.str_dup(base_path,
    ...
    char *get_fullpath(uint64_t root,
        ...
        return ct_path_v0.join(allocator, 2, root_path, filename);
    }
    ...
    struct ct_vio *open(uint64_t root,
        ...
        struct ct_vio *file = ct_vio_api_v0.from_file(full_path,
                                                      (ct_vio_open_mode) mode);

        if (!file) {
            ct_log_api_v0.error(LOG_WHERE, "Could not load file %s", full_path);
    ...
    int create_directory(uint64_t root,
        ...
        int ret = ct_path_v0.make_path(full_path);
        CETECH_FREE(a, full_path);
    ...
    void listdir(uint64_t root,
        ...
        ct_path_v0.list(full_path, 1, files, count, allocator);
    ...
    void listdir_free(char **files,
                      uint32_t count,
                      struct ct_allocator *allocator) {
        ct_path_v0.list_free(files, count, allocator);
    }
    ...
    time_t get_file_mtime(uint64_t root,
        ...
        time_t ret = ct_path_v0.file_mtime(full_path);
...
namespace filesystem_module {
    static struct ct_filesystem_api_v0 _api = {
    ...
    void _init_api(struct ct_api_v0 *api) {
        api->register_api("ct_filesystem_api_v0", &_api);
    }
    ...
    void _init(struct ct_api_v0 *api) {
        ...
        _G.root_map.init(ct_memory_api_v0.main_allocator());
    ...
    void _shutdown() {
        ...
        while (it != end_it) {
            CETECH_FREE(ct_memory_api_v0.main_allocator(), it->value);
            ++it;
        }
    ...
    extern "C" void filesystem_load_module(struct ct_api_v0 *api) {
        _init(api);
    }

    extern "C" void filesystem_unload_module(struct ct_api_v0 *api) {
        _shutdown();
    }
}