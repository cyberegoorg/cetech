...
CETECH_DECL_API(ct_level_api_v0);
CETECH_DECL_API(ct_hash_api_v0);

static int _load_level(lua_State *l) {
    world_t world = {.h = luasys_to_handler(l, 1)};
    uint64_t name = ct_hash_api_v0.id64_from_str(luasys_to_string(l, 2));


    level_t level = ct_level_api_v0.load_level(world, name);
...
static int _destroy_level(lua_State *l) {
    ...
    ct_level_api_v0.destroy(world, level);
    return 0;
}
...
static int _level_entity_by_id(lua_State *l) {
    ...
    entity_t ent = ct_level_api_v0.entity_by_id(level, name);
...
static int _level_entity(lua_State *l) {
    ...
    entity_t ent = ct_level_api_v0.entity(level);

    luasys_push_uint32_t(l, ent.h);
    return 1;
}

void _register_lua_level_api(struct ct_api_v0 *api) {
    CETECH_GET_API(api, ct_level_api_v0);
