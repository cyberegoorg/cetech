#include <cetech/celib/allocator.h>
#include <cetech/kernel/hash.h>
#include <cetech/modules/entity.h>
#include <cetech/kernel/module.h>
#include <cetech/modules/luasys.h>
#include <cetech/kernel/api_system.h>

#define API_NAME "Entity"

CETECH_DECL_API(ct_entity_api_v0);
CETECH_DECL_API(ct_hash_api_v0);

static int _entity_spawn(lua_State *l) {
    world_t w = {.h = luasys_to_handler(l, 1)};
    const char *name = luasys_to_string(l, 2);

    uint64_t nameid = ct_hash_api_v0.id64_from_str(name);

    luasys_push_handler(l, ct_entity_api_v0.spawn(w, nameid).h);
    return 1;
}

static int _entity_destroy(lua_State *l) {
    world_t w = {.h = luasys_to_handler(l, 1)};
    entity_t entity = {.h = luasys_to_handler(l, 2)};

    ct_entity_api_v0.destroy(w, &entity, 1);
    return 1;
}


void _register_lua_entity_api(struct ct_api_v0 *api) {
    CETECH_GET_API(api, ct_entity_api_v0);
    CETECH_GET_API(api, ct_hash_api_v0);

    luasys_add_module_function(API_NAME, "spawn", _entity_spawn);
    luasys_add_module_function(API_NAME, "destroy", _entity_destroy);
}