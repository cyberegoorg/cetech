...
CETECH_DECL_API(ct_module_api_v0);

static int _reload_all(lua_State *l) {
    ct_module_api_v0.module_reload_all();
    return 0;
}

static int _reload(lua_State *l) {
    const char *path = luasys_to_string(l, 1);
    ct_module_api_v0.module_reload(path);
    return 0;
}


void _register_lua_module_api(struct ct_api_v0 *api) {
    CETECH_GET_API(api, ct_module_api_v0);
