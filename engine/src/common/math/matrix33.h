...
namespace cetech {
    ...
    namespace matrix33 {
	CE_INLINE Matrix33 make_matrix33(const Vector3& x, const Vector3& y, const Vector3& z);
	...
	Matrix33 from_axis_angle ( const Vector3& axis, const float angle_deg ) {
	    float sin, cos, C;
		...
		{(axis.x * axis.x * C) + cos, (axis.x * axis.y * C) - (axis.z * sin), (axis.x * axis.z * C) + (axis.y * sin)},
		{(axis.y * axis.x * C) + (axis.z * sin), (axis.y * axis.y * C) + cos, (axis.y * axis.z * C) - (axis.x * sin)},
