...
namespace cetech {
    namespace device_internal {
        ...
        void init_boot() {
            ...
            device_globals::device().resource_manager()->load(package_manager::type_name(), boot_pkg_name_h);
        ...
        void compile_all_resource() {
            ...
            for (uint32_t i = 0; i < files_count; ++i) {
                ...
                device_globals::device().resource_manager()->compile(path_base);
            }
        }
        ...
        void register_resources() {
            ...
            while (it->type != 0) {
                device_globals::device().resource_manager()->register_unloader(it->type, it->unloader);
                device_globals::device().resource_manager()->register_loader(it->type, it->loader);
                device_globals::device().resource_manager()->register_compiler(it->type, it->compiler);
                ++it;
            }
        }
    }
    ...
    namespace device_globals {
        ...
        void init() {
            ...
            _device = MAKE_NEW(memory_globals::default_allocator(), DeviceImplementation);
        }
