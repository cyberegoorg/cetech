...
namespace module {
    ...
    void load(const char *path) {
        ct_log_api_v0.info(LOG_WHERE, "Loading module %s", path);
...
//                data = api->reload_begin(&ct_api_v0);
//            }
//
...
//        }
    ...
    void load_dirs(const char *path) {
        ...
        ct_path_v0.list(path, 1, &files, &files_count,
                        ct_memory_api_v0.main_allocator());

        for (int k = 0; k < files_count; ++k) {
            const char *filename = ct_path_v0.filename(files[k]);
    ...
    void unload_all() {
        for (int i = MAX_PLUGINS - 1; i >= 0; --i) {
            ...
            _G.unload_module[i](&ct_api_v0);
        }
    }


    static ct_module_api_v0 module_api{
    ...
    void init(struct ct_allocator *allocator,
        ...
        api->register_api("ct_module_api_v0", &module_api);
