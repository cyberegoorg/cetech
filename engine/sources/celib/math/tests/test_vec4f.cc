    ...
    GIVEN( "v1 = [1.0f, 2.0f, 3.0f, 4.0f] and v2 = [5.0f, 6.0f, 7.0f]" ) {
        ...
        WHEN("normalize v1") {
            ...
            THEN("result == [~0.18257f, ~0.36515f, ~0.54772f, ~0.7303f]") {
                REQUIRE(result.f[0] == Approx(0.18257f));
                REQUIRE(result.f[1] == Approx(0.36515f));
                REQUIRE(result.f[2] == Approx(0.54772f));
                REQUIRE(result.f[3] == Approx(0.7303f));
            }
        }

        WHEN("v1 eq v2") {
            THEN("v1 != v2") {
                REQUIRE_FALSE(vec4f_eq(&v1, &v2, f32_Epsilon));
            }
        }

        WHEN("v1 eq v1") {
            THEN("v1 == v1") {
                REQUIRE(vec4f_eq(&v1, &v1, f32_Epsilon));
            }
        }
