...
static int _unit_x(lua_State *l) {
    luasys_push_vector2(l, VEC2F_UNIT_X);
    return 1;
}

static int _unit_y(lua_State *l) {
    luasys_push_vector2(l, VEC2F_UNIT_Y);
    return 1;
}

static int _length(lua_State *l) {
    vec2f_t *v = luasys_to_vector2(l, 1);
...
static int _lerp(lua_State *l) {
    vec2f_t *from = luasys_to_vector2(l, 1);
    vec2f_t *to = luasys_to_vector2(l, 2);
