...
void llm_dir_list(const char *path,
                  int recursive,
...
void llm_dir_list_free(ARRAY_T(pchar) *files, struct allocator *allocator) {
    for (int i = 0; i < ARRAY_SIZE(files); ++i) {
        CE_DEALLOCATE(allocator, ARRAY_AT(files, i));
    }
}

int llm_dir_make(const char *path) {
    struct stat st;
    const int mode = 0775;
...
int llm_dir_make_path(const char *path) {
    char *pp;
    char *sp;
    int status = 1;
